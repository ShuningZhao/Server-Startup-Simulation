...
% Project 1 random simulation 
% by Shuning Zhao z3332916

function [all_arrival_time, all_departure_time, avg_response_time] = sim_random_func(lambda,mu,m,setup_time, delayedoff_time, time_end)
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Constants
%%%%%%%%%%%%%%%%%%%%%%%%%%%
UNMARKED = 0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% Initialising arrival and service times
% Estimated number of arrivals and services needed.
test_length = round(1.5 * lambda * time_end);
arrival_times = -log(1-rand(test_length,1))/lambda;
all_arrival_time_file = [];
i = 1;
value = 0;
while value < time_end
    value = sum(arrival_times(1:i));
    all_arrival_time_file = [all_arrival_time_file;value];
    i = i + 1;
end
all_arrival_time_file = all_arrival_time_file(all_arrival_time_file < time_end);
arrival_length = length(all_arrival_time_file);
all_service_time_file = -log(1-rand(arrival_length, 1))/mu-log(1-rand(arrival_length, 1))/mu-log(1-rand(arrival_length, 1))/mu;
task_index = 1;
